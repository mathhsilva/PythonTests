...
class Application:
        ...
        if not self.usuario_existe(login_digitado):
            if self.validar_senha() and senha_digitada:
                try:
                    ...
                    cursor.execute("INSERT INTO usuario (codprod, descprod, prcusto, prvenda, qtd, datacad) VALUES (nextval('seq_produto_id'), %s, %s, %s, %s, %s)", (nome_digitado, login_digitado, senha_digitada, qtde_digitada, data_formatada))
                    conexao.commit()
    ...
    def criar_interface(self):
        ...
        # Frame Widgets
        label3 = customtkinter.CTkLabel(master=self.janela, text="Cadastro de Produtos", font=("Roboto", 25, "bold"))
        label3.place(relx=0.5, rely=0.05, anchor="center")

        # Criando campo de entrada para o nome
        self.ednome = customtkinter.CTkEntry(master=frame, placeholder_text="Nome do Produto", width=300, font=("Robot o", 14))
        self.ednome.place(x=25, y=65)

        # Criando campo de entrada para o login
        self.edlogin = customtkinter.CTkEntry(master=frame, placeholder_text="Preço de Custo", width=300, font=("Robot o", 14))
        self.edlogin.place(x=25, y=105)

        # Criando campo de entrada para a senha
        self.edsenha = customtkinter.CTkEntry(master=frame, show="*", placeholder_text="Preço de Venda", width=300, font=("Roboto", 14))
        ...
        # Confirmando a senha
        self.edqtde = customtkinter.CTkEntry(master=frame, show="*", placeholder_text="Informe a quantidade", width=300, font=("Roboto", 14))
        self.edqtde.place(x=25, y=185)
